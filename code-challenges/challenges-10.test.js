...
const getNames = (arr) => {
  // Solution code here...
let newA =  arr.map( element=> element.name.split('').reverse().join(''));
return newA;
};
...
let findMaleAndFemale = (data) => {
  ...
  }).map(element1 =>element1.name).join(' and ');
  return newArray;
};
...
let findShortest = (data) => {
  // Solution code here...
  let newArray = data.sort((elem1,elem2) => elem1.height - elem2.height ).map(ele=>ele.name)
  return newArray[0];
};
